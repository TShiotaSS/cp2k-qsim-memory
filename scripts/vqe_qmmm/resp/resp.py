...
from vqe_qmmm.resp.resp_helper import *
...
def generate_surface(mol, doDebug=False):
    ...
    for i in range(n_vdw_layers):
        ...
        surface.vdw_surface(mol.atom_coords()*bohr_to_angstrom,
                            mol.elements, scale_factor,
                            vdw_point_density, radii)
...
def espfit(inv_riJ, esp_values, q_tot):
    ...
    #print ("niter", niter)
    if difm > toler:
        print ("Warning: Not Converged!! Try increasing the maximum number of iterations")
    return q
...
def compute_resp(mf, cwf=None):
    ...
    #print (points.shape)
    mf_qmmm = qmmm.mm_charge(mf, points,
                             np.zeros_like(points[:,0]), unit='Bohr')
