...
def vpot_qm_nuc(mf):
    ...
    charges = np.ones_like(mf.mol.atom_charges(),
                           dtype='float64')
    vnuc_qm = np.zeros_like(charges,
                            dtype='float64')
...
def vpot_qm_elec(mf, cwf=None):
    ...
    charges = np.ones_like(mf.mol.atom_charges(),
                           dtype='float64')
    vpot_qm_elec = np.zeros_like(charges,
                                 dtype='float64')
    if cwf is None:
        dm = mf.make_rdm1()
    else:
...
def write_vpot(mf_qmmm, cwf=None, filename='vpot.out', fmt='%22.15f'):
    ...
    for (x, y, z), q in zip(coords, vpot):
...
def field_mm(mf, cwf=None):
    ...
    charges = np.ones_like(mf.mm_mol.atom_charges(),
                           dtype='float64')
